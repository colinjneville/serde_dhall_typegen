...
use super::common::{chunk_size, chunk_start, index_chunk};
...
impl<T> IntoIterator for AppendList<T> {
    ...
    fn into_iter(self) -> Self::IntoIter {
        let vecs = self.chunks.into_inner();
        let f: std::iter::Flatten<std::vec::IntoIter<Vec<T>>> = vecs.into_iter().flatten();
        IntoIter(f)
    }
}
...
pub struct IntoIter<T>(std::iter::Flatten<std::vec::IntoIter<Vec<T>>>);
